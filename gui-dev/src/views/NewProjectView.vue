    ...
    <div class="flex-1 p-10">
      ...
      <div class="mt-8 space-y-6">
        ...
        <div v-show="activeSection === 'structure'" class="space-y-6">
            ...
            <!-- Project Structure Editor - Handles all project types -->
            <ProjectStructureEditor
              v-if="settingsCatalog"
              :project-type="project.type"
              :directories="project.directories"
              :render-dirs="project.render_dirs"
              :enabled="project.directories_enabled"
              :extra-directories="allProjectExtraDirectories"
              :settings="project"
              :gitignore="project.git.gitignore_content"
              :catalog="settingsCatalog.project_types?.[project.type]"
              :new-extra-ids="newExtraIds"
              :allow-destruction="true"
              @update:directories="project.directories = $event"
              @update:render-dirs="project.render_dirs = $event"
              @update:enabled="project.directories_enabled = $event"
              @update:extra-directories="updateAllExtraDirectories"
              @update:gitignore="project.git.gitignore_content = $event"
...
// Reactive ref for tracking new extra directories
...
// Computed: All extra directories for ProjectStructureEditor
const allProjectExtraDirectories = computed(() => {
  return Array.isArray(project.value.extra_directories) ? project.value.extra_directories : []
})

// Handler: Update all extra directories from ProjectStructureEditor
const updateAllExtraDirectories = (updatedDirectories) => {
  project.value.extra_directories = updatedDirectories
}

// Handler: Reset project structure to defaults
const resetToDefaults = () => {
  if (!settingsCatalog.value?.project_types?.[project.value.type]) return

  const catalog = settingsCatalog.value.project_types[project.value.type]

  // Reset directories to catalog defaults
  if (catalog.directories) {
    project.value.directories = {}
    Object.keys(catalog.directories).forEach(key => {
      project.value.directories[key] = catalog.directories[key].default
    })
  }

  // Reset enabled states to catalog defaults
  if (catalog.directories) {
    project.value.directories_enabled = {}
    Object.keys(catalog.directories).forEach(key => {
      project.value.directories_enabled[key] = catalog.directories[key].enabled_by_default === true
    })
  }
  ...
  // Reset gitignore to catalog default
  if (catalog.gitignore_template) {
    project.value.git.gitignore_content = catalog.gitignore_template
  }
}
