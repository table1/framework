...
init_global_config <- function(force = FALSE) {
  ...
  # Attempt migration if config dir is empty (interactive only)
  if (!file.exists(settings_path) && !force && interactive()) {
...
.migrate_config_from_previous <- function(target_dir) {
  ...
  if (dir.exists(legacy_dir) && file.exists(file.path(legacy_dir, "settings.yml"))) {
    ...
    .copy_config_files(legacy_dir, target_dir)
    return(TRUE)
  }
...
#' @keywords internal
.copy_config_files <- function(from_dir, to_dir) {
  # Copy config files
  files_to_copy <- c("settings.yml", "projects.yml", "settings-catalog.yml")
  for (f in files_to_copy) {
    src <- file.path(from_dir, f)
    if (file.exists(src)) {
      file.copy(src, file.path(to_dir, f), overwrite = FALSE)
    }
  }
  ...
  if (dir.exists(templates_src)) {
    ...
    for (tf in template_files) {
      file.copy(tf, templates_dst, overwrite = FALSE)
    }
  }
}
