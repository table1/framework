...
test_that("can load SPSS files directly", {
  df <- data_read(test_path("fixtures/test.sav"))
  expect_s3_class(df, "data.frame")
  expect_equal(nrow(df), 5)
  expect_equal(ncol(df), 4)
})
...
test_that("can load SAS transport files directly", {
  df <- data_read(test_path("fixtures/test.xpt"))
  expect_s3_class(df, "data.frame")
  expect_equal(nrow(df), 5)
  expect_equal(ncol(df), 4)
})
...
test_that("preserves haven attributes when keep_attributes = TRUE", {
  df <- data_read(test_path("fixtures/test.dta"), keep_attributes = TRUE)
...
test_that("validates keep_attributes parameter", {
  expect_error(
    data_read(test_path("fixtures/test.dta"), keep_attributes = "yes"),
    "Assertion on 'keep_attributes' failed"
  )

  expect_error(
    data_read(test_path("fixtures/test.dta"), keep_attributes = NULL),
    "Assertion on 'keep_attributes' failed"
  )
})
...
test_that("passthrough parameters work with haven functions", {
  ...
  df <- data_read(test_path("fixtures/test.dta"), encoding = "UTF-8")
  expect_s3_class(df, "data.frame")
})
